...
var ssListHandle *sslist.SSList
...
func init() {
	...
	ssListHandle, err = sslist.Open(ssdbHandle, "log")
	if err != nil {
		log.Fatalf("Open Failed")
	}
}
...
func TestGetRange(t *testing.T) {
	dbrange := ssdbHandle.NewDBRange("log", 2, 0, 1, 9999)
	_data := ssListHandle.GetRange(dbrange)
	fmt.Println(_data)
}
